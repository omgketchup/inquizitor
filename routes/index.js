...
exports.apphome = function(req, res) {
    if (req.isAuthenticated()) {
        ...
        res.render('login', {
            status: 'failure',
            message: 'Sorry, something didnt work out there'
        });
    }
}
...
exports.uploadimage = function(req, res) {
    if (!req.isAuthenticated()) {
        res.send({
            status: 'failure',
            message: 'Failed authentication before upload.'
        });
        return;
    }
...
exports.signup = function(req, res) {
    if (req.method == "POST") {
        ...
        if (valErrors) {
            ...
            res.render('signup', {
                status: "failure",
                message: "Invalid email address or password " + valErrors,
                css: "alert"
            });
            return;
        }
        ...
        User.findOne({
            ...
            if (user == null) {
                ...
                res.render('signup', {
                    status: "failure",
                    message: "That email address already exists in our system",
                    css: "alert"
                })
            }
        });
...
exports.copyquiz = function(req, res) {
    ...
    if (req.isAuthenticated()) {
        SweetQuiz.findOne({
            ...
            if (err) {
                console.log("Error while finding quiz: " + err);
                return;
            } else {
                if (foundQuiz != null) {
                    ...
                    theCopy.save(function(err) {
                        if (err) {
                            ...
                            res.send({
                                status: "failure",
                                message: "Couldn't copy quiz."
                            })
                            return;
                        } else {
                            res.send({
                                status: "success",
                                message: "Successfully copied that quiz."
                            });
                        }
                    });
                }
        ...
        SweetQuiz.update({
            ...
            function(err, fin, details, extra) {
                ...
                if (err) {
                    res.send({
                        status: "failure",
                        message: "Couldn't update that quiz. " + err
                    });
                } else {
                    //console.log("Success");
                    ...
                    res.send({
                        ...
                        data: {
                            id: details.upserted,
                            updatedExisting: details.updatedExisting
                        },
            ...
            if (q != null) {
                if (req.isAuthenticated()) {
                    if (req.user.email == q.author) {
                        QuizResponse.find({
                            ...
                            function(err, foundResponses) {
                                if (err) {
                                    res.send({
                                        status: "success",
                                        message: "Got that quiz, what an accomplishment!",
                                        quiz: q
                                    });
                                    return;
                                } else {
                                    if (foundResponses != null && typeof(foundResponses) != 'undefined' && foundResponses.length != 0) {
                                        ...
                                        res.send({
                                            status: "success",
                                            message: "Got that quiz, what an accomplishment! " + q.numba,
                                            quiz: q
                                        });
                                    } else {
                                        res.send({
                        ...
                        res.send({
                            status: "success",
                            message: "Got quiz, no responses or anything",
                            quiz: q
                        });
                    }
                } else {
                ...
                res.send({
                    status: "failure",
                    message: "No quizzes matched that query."
                });
            }
        }
    );
    ...
    if (req.isAuthenticated()) {
        ...
        SweetQuiz.findOne({
            ...
            if (err) {
                ...
                res.send({
                    status: 'failure',
                    message: 'Error deleting quiz ' + err
                });
                return;
            } else {
        ...
        res.send({
            status: "failure",
            message: "Not authorized to delete that quiz."
        });
    }
}
exports.viewquiz = function(req, res) {
    ...
    SweetQuiz.findOne({
        ...
        if (err) {
            console.log("Error looking up quiz for viewing... " + err);
            return;
        }
        //console.log("Finished FindOne for a quiz" + quiz);
        if (quiz != null) {
            ...
            res.send({
                status: 'success',
                data: quiz,
                message: 'Got a quiz for viewing, no answer info...'
            });
        } else {
            res.send({
                status: 'failure',
                message: 'No quiz matching that ID was found. '
            });
        }
    });
}
...
exports.postresponse = function(req, res) {
    ...
    if (response.email != response.cemail) {
        res.send({
            status: 'failure',
            message: 'Email and email confirmation did not match.'
        });
        return;
    }
                    ...
                    quizResponse.save(
                        function(err, saved) {
                            if (err) {
                                ...
                                res.send({
                                    status: 'failure',
                                    message: "Got an error while saving a multiple response: " + err
                                });
                            } else {
                            ...
                            function(err, foundQuizResponse) {
                                if (err) {
                                    ...
                                    if (foundQuizResponse && foundQuizResponse != null) {
                                        ...
                                        res.send({
                                            status: "failure",
                                            message: "You have already submitted a response to this quiz, you cannot submit another."
                                        });
                                    } else {
                                        var qresponse = new QuizResponse(response);
                                        qresponse.submitted = Date.now();
    ...
    if (req.isAuthenticated()) {
        ...
        SweetQuiz.findOne({
            ...
            if (err) {
                ...
                res.send({
                    status: 'failure',
                    message: 'Sorry, could not find a quiz to match that ID.'
                });
                return;
            } else {
                if (foundQuiz != null) {
                    if (foundQuiz.author == req.user.email) {
                        QuizResponse.find({
                            ...
                            function(err, foundResponses) {
                                if (err) {
                                    console.log("Looked up responses to the legit quiz, but got an error: " + err);
                                    return;
                                }
                                if (foundResponses != null) {
                                    ...
                                    res.send({
                                        status: 'failure',
                                        message: "Sorry, no responses found for that."
                                    });
                                }
                            }
                        );
                        ...
                        res.send({
                            status: 'failure',
                            message: "Sorry, but you're not allowed to see those responses."
                        });
                    }
                } else {
                    res.send({
                        status: 'failure',
                        message: "Sorry, that doesn't seem to be a valid thing to ask for."
                    });
                }
            }
        })
        ...
        res.send({
            status: 'failure',
            message: "Sorry, you need to be logged in to do that."
        });
    }
}
        ...
        function(err, foundResponse) {
            if (err) {
                ...
                if (foundResponse == null || !foundResponse || typeof(foundResponse) == 'undefined') {
                    res.send({
                        status: 'failure',
                        message: 'foundResponse was null'
                    });
                    return;
                }
                SweetQuiz.findOne({
                    _id: foundResponse.responseTo
                }, function(err, foundQuiz) {
                    if (err) {
                        res.send({
                            status: 'failure',
                            message: 'no quiz found for this response'
                        });
                        return;
                    } else {
                        if (!foundQuiz) {
...
exports.takebadquiz = function(req, res) {
    res.render('templates/error', {
        status: 'failure',
        message: 'That thing you wanted... we could not find it.'
    });
}
